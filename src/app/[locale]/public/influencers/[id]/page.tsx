...
export default async function InfluencerPublicProfilePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (!user) {
    return (
      <>
        <SiteHeader />
        <main className="flex-1 bg-mesh"><div className="mx-auto max-w-3xl px-4 py-16 text-[var(--foreground-muted)]">Influencer not found.</div></main>
        <SiteFooter />
      </>
    );
  }
              ...
              <Card className="animate-fade-in-up">
                <CardHeader>
                  <div className="flex items-start justify-between gap-4">
                    <div className="flex items-center gap-4">
                      <div className="flex h-16 w-16 items-center justify-center rounded-full bg-[var(--primary)] text-2xl font-bold text-[var(--background)] shadow-lg shadow-[var(--primary-glow)]">
                ...
                <CardContent className="space-y-4">
                  ...
                  {user.influencerProfile?.passion && (
                    <div className="text-sm"><span className="font-medium text-[var(--foreground)]">Passion:</span> {user.influencerProfile.passion}</div>
                  )}
                  {user.influencerProfile?.followersCountRange && (
                    <div className="text-sm"><span className="font-medium text-[var(--foreground)]">Followers:</span> {user.influencerProfile.followersCountRange}</div>
                  )}
                  {user.influencerProfile?.mainAccountLink && (
                    <div className="text-sm"><span className="font-medium text-[var(--foreground)]">Main account:</span>{" "}
                      <a href={user.influencerProfile.mainAccountLink} target="_blank" rel="noopener noreferrer" className="text-[var(--primary)] hover:underline">{user.influencerProfile.mainAccountLink}</a>
                    </div>
                  )}
                  {user.influencerProfile?.socialNetworks && (
                    <div className="text-sm"><span className="font-medium text-[var(--foreground)]">Social networks:</span> {user.influencerProfile.socialNetworks}</div>
                  )}
                  ...
                  {ratings.map((r) => (
                    <div key={r.id} className="border-b border-[var(--border)] pb-3 last:border-0">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-xs text-[var(--foreground-muted)]">{r.reviewer.firstName} {r.reviewer.lastName} ({r.reviewer.role})</span>
                      </div>
            ...
            <div className="md:col-span-2 space-y-6">
              ...
              <Card className="animate-fade-in-up">
                ...
                <CardContent className="space-y-3">
                  {user.participations.length === 0 && <div className="text-sm text-[var(--foreground-muted)]">No completed campaigns yet.</div>}
                  {user.participations.map((p) => (
                    <div key={p.id} className="flex items-center gap-3 rounded-lg border border-[var(--border)] p-2">
                      {p.campaign.photoUrl ? (
                      ...
                      <div className="text-sm font-medium text-[var(--foreground)]">{p.campaign.title}</div>
                    </div>
                  ))}
                </CardContent>
              </Card>
