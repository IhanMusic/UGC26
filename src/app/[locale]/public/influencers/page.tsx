      ...
      <main className="flex-1 bg-mesh">
        <div className="mx-auto max-w-6xl px-4 py-16">
          <div className="animate-fade-in-up">
            ...
            <p className="mt-2 text-slate-500">Discover talented creators for your campaigns.</p>
          </div>
          ...
          <form className="mt-8 flex flex-wrap items-end gap-3" action="">
            <div className="space-y-1">
              ...
              <input name="q" defaultValue={q} placeholder="Name…" className="h-10 rounded-xl border border-slate-200/60 bg-white/50 px-4 text-sm shadow-sm backdrop-blur-sm focus:border-violet-300 focus:outline-none focus:ring-2 focus:ring-violet-400/50" />
            </div>
            <div className="space-y-1">
            ...
            <div className="space-y-1">
              ...
              <input name="city" defaultValue={city} placeholder="City…" className="h-10 rounded-xl border border-slate-200/60 bg-white/50 px-4 text-sm shadow-sm backdrop-blur-sm focus:border-violet-300 focus:outline-none focus:ring-2 focus:ring-violet-400/50" />
            </div>
            <div className="space-y-1">
              <label className="text-xs font-medium text-slate-500">Country</label>
              <input name="country" defaultValue={country} placeholder="Country…" className="h-10 rounded-xl border border-slate-200/60 bg-white/50 px-4 text-sm shadow-sm backdrop-blur-sm focus:border-violet-300 focus:outline-none focus:ring-2 focus:ring-violet-400/50" />
            </div>
          ...
          <div className="mt-10 grid gap-5 md:grid-cols-3">
            {users.length === 0 && (
              <div className="col-span-full text-center text-slate-400 py-12">No influencers found.</div>
            )}
            {users.map((u) => {
                ...
                <Card key={u.id} className="group overflow-hidden animate-fade-in-up">
                  <CardHeader>
                    <div className="flex items-center gap-3">
                      <div className="flex h-12 w-12 items-center justify-center rounded-full bg-gradient-to-br from-violet-500 to-indigo-500 text-lg font-bold text-white shadow-lg shadow-violet-500/25">
                        {u.firstName.charAt(0)}
                      </div>
                      <div>
                        <CardTitle className="text-base">
                          <Link href={`/public/influencers/${u.id}`} className="hover:text-violet-700 hover:underline underline-offset-4">
                  ...
                  <CardContent className="space-y-3">
                    ...
                    {u.influencerProfile?.passion && (
                      <div className="text-xs text-slate-500">Passion: {u.influencerProfile.passion}</div>
                    )}
                    ...
                    {u.influencerProfile?.followersCountRange && (
                      <div className="text-xs text-slate-500">Followers: {u.influencerProfile.followersCountRange}</div>
                    )}
                  </CardContent>
                </Card>
              );
