...
export default function MessagesPage() {
  ...
  return (
    <div className="flex h-[calc(100vh-120px)] overflow-hidden rounded-2xl border border-white/20 bg-white/60 backdrop-blur-2xl">
              ...
              return (
                <button
                  ...
                  className={cn(
                    ...
                    hasUnread && "bg-violet-50/30"
                  )}
                >
                  <div className="flex h-10 w-10 flex-shrink-0 items-center justify-center rounded-full bg-gradient-to-br from-violet-500 to-indigo-500 text-sm font-bold text-white">
                    {other?.firstName?.charAt(0)?.toUpperCase() ?? "?"}
                  </div>
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center justify-between">
                      <span className={cn("text-sm font-medium truncate", hasUnread ? "text-slate-900" : "text-slate-600")}>
                    ...
                    {conv.campaign && (
                      <div className="text-[10px] text-violet-500 truncate">{conv.campaign.title}</div>
                    )}
                    {conv.lastMessage && (
                      <p className={cn("mt-0.5 text-xs truncate", hasUnread ? "font-medium text-slate-700" : "text-slate-400")}>
          ...
          <>
            ...
            <div className="flex items-center gap-3 border-b border-slate-100 px-4 py-3">
              ...
              <div>
                ...
                {activeConv?.campaign && (
                  <div className="text-[10px] text-violet-500">{activeConv.campaign.title}</div>
                )}
              </div>
            </div>
            ...
            <form onSubmit={handleSend} className="border-t border-slate-100 px-4 py-3">
              <div className="flex gap-2">
                <input
                  ...
                  className="flex-1 rounded-xl border border-slate-200 bg-white px-4 py-2.5 text-sm outline-none transition-colors focus:border-violet-400 focus:ring-2 focus:ring-violet-100"
                  disabled={sending}
                />
                <button
                  type="submit"
                  disabled={!newMsg.trim() || sending}
                  className="flex h-10 w-10 items-center justify-center rounded-xl bg-gradient-to-br from-violet-600 to-indigo-600 text-white shadow-md shadow-violet-500/25 transition-all hover:shadow-lg disabled:opacity-50"
                >
